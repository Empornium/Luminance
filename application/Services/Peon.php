...
class Peon extends Service {
    ...
    protected function get_base_variables () {
        ...
        if ($user) {
            ...
            if (is_dir($tplPath)) {
                $this->tpl->override_template($tplPath);
            }
        }
    ...
    public function display_page($template, $values) {
        ...
        $this->tpl->display($template, $values);
    }
           ...
           if ($user->legacy['SupportFor'] !="" || $this->permissions->load($user->legacy['PermissionID'])->DisplayStaff == 1) {
               ...
               $hv->NumUnansweredStaffPMs = $this->db->raw_query("SELECT COUNT(ID) FROM staff_pm_conversations
                            WHERE (AssignedToUser=:userid OR Level <=:userclass)
                              ...
                              [':userid'    => $user->ID,
                               ':userclass' => $this->permissions->load($user->legacy['PermissionID'])->Level])->fetchColumn();
