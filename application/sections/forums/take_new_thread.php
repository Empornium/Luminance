...
if (empty($_POST['question']) || empty($_POST['answers']) || !check_perms('site_polls_create')) {
    $NoPoll = 1;
} else {
    $NoPoll = 0;
    ...
    if ($ForumID == STAFF_FORUM_ID) {
        send_irc("PRIVMSG ".ADMIN_CHAN." :!mod Poll created by ".$LoggedUser['Username'].": '".$Question."' http://".SITE_URL."/forums.php?action=viewthread&threadid=".$TopicID);
    }
}
