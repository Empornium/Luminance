...
class Debug
{
    ...
    public function analysis($Message, $Report='', $Time=43200)
    {
        ...
        send_irc('PRIVMSG '.LAB_CHAN.' :'.$Message.' '.$Document.' '.' http://'.SITE_URL.'/tools.php?action=analysis&case='.$Identifier.' http://'.SITE_URL.$_SERVER['REQUEST_URI']);
    }
    ...
    public function get_cache_times()
    {
        global $Cache;

        return $Cache->CacheTimes;
    }
    ...
    public function get_queries()
    {
        ...
        return array_merge($master->olddb->Queries, $master->db->Queries);
    }
...
<?php
    ...
    public function cache_table($CacheKeys=false)
    {
        ...
        if (!is_array($CacheKeys)) {
            ...
            $CacheTimes = $this->get_cache_times();
...
<?php	foreach ($CacheKeys as $Key) { ?>
        <tr>
            ...
            <td class="rowa" style="width:50px;" align="left"><a href="/tools.php?action=clear_cache&amp;key=<?=$Key?>&amp;type=clear&amp;auth=<?=$LoggedUser['AuthKey']?>" title="clear <?=$Key?>">clear</a></td>
        </tr>
<?php	} ?>
    </table>
<?php
