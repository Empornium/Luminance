                ...
                if ($ValidateVar!="" || !empty($Field['Required']) || $Field['Type']=="date") {
                    switch ($Field['Type']) {
                        ...
                        case 'email':
                            ...
                            if ($result !== true) {
                                return "$Field[ErrorMessage]<br/>".display_str($result);
                            }
                            break;
                        ...
                        case 'checkbox':
                            if (!isset($ValidateArray[$Field['Name']])) {
                                return $Field['ErrorMessage'];
                            }
                            break;
                        ...
                        case 'inarray':
                        case 'enum':
                            if (!is_array($Field['InArray'])) {
                                return $Field['ErrorMessage'] .' (InArray is not an array)';
                            }
                        ...
                        case 'desc':
                            ...
                            if ($num && $num >= $MinImages) { // if there are no img tags then it validates
                                ...
                                for ($j=0; $j<$num; $j++) {
                                    ...
                                    if ($result !== true) {
                                        return $Field['ErrorMessage'].' field: ' .display_str($result);
                                    }
        ...
        foreach ($this->Fields as $Field) {
            if ($Field['Type']=="string") {
                ...
                if (!empty($Field['MaxLength'])) {
                    $ValItem.=' || $(\'#'.$Field['Name'].'\').raw().value.length>'.$Field['MaxLength'];
                } else {
                    $ValItem.=' || $(\'#'.$Field['Name'].'\').raw().value.length>255';
                }
                if (!empty($Field['MinLength'])) {
                    $ValItem.=' || $(\'#'.$Field['Name'].'\').raw().value.length<'.$Field['MinLength'];
                }
                ...
                if (!empty($Field['MaxLength'])) {
                    $ValItem.=' || $(\'#'.$Field['Name'].'\').raw().value/1>'.$Field['MaxLength'];
                }
                if (!empty($Field['MinLength'])) {
                    $ValItem.=' || $(\'#'.$Field['Name'].'\').raw().value/1<'.$Field['MinLength'];
                }
                $ValItem.=') { return showError(\''.$Field['Name'].'\',\''.$Field['ErrorMessage'].'\'); }'."\r\n";
                ...
                if (!empty($Field['MinLength'])) {
                    ...
                    $ValItem.=' || $(\'#'.$Field['Name'].'\').raw().value.length<6';
                }
                $ValItem.=') { return showError(\''.$Field['Name'].'\',\''.$Field['ErrorMessage'].'\'); }'."\r\n";
                ...
                if (!empty($Field['MinLength'])) {
                    ...
                    $ValItem.=' || $(\'#'.$Field['Name'].'\').raw().value.length<10';
                }
                $ValItem.=') { return showError(\''.$Field['Name'].'\',\''.$Field['ErrorMessage'].'\'); }'."\r\n";
                ...
                if (isset($Field['MinLength']) && $Field['MinLength']==3) {
                    $Day='$(\'#'.$Field['Name'].'day\').raw().value';
                    $DisplayError.=",".$Field['Name']."day";
                } else {
                    $Day="1";
                }
                ...
                if (empty($Field['Required'])) {
                    ...
                    if (isset($Field['MinLength']) && $Field['MinLength']==3) {
                        $ValItem.=' || $(\'#'.$Field['Name'].'day\').raw().value!=""';
                    }
