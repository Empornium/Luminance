...
if (!check_perms('site_play_slots')) {
    ajax_error("You do not have permission to play the xxx slot machine!");
}
...
if ($FloodCheck !== false) {
    ajax_error("You must wait 5 secs before playing again");
}
...
if (!in_array($BetAmount, array(1,10,100))) {
    ajax_error('You can only bet valid values: 1, 10, or 100');
}
...
if ($LoggedUser['TotalCredits']<$TotalBet) {
    ajax_error("Not enough credits to bet ".number_format($TotalBet));
}
...
// record spins
$DB->query("INSERT INTO sm_results (UserID, Won, Bet, Spins, Time)
                  VALUES ( '$UserID','$Win','$TotalBet','$NumBets','".sqltime()."')
                  ON DUPLICATE KEY UPDATE
                  Won=Won+'$Win', Bet=Bet+'$TotalBet', Spins=Spins+'$NumBets', Time='".sqltime()."'");
