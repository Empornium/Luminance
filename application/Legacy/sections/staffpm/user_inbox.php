...
if ($Assign !== '' && !in_array($Assign, array('mod','smod','admin'))) {
    $Assign = '';
}
...
<?php
    ...
    $UrgentStr='';
    if ($Urgent != 'No') {
        if ($Urgent == 'Read' && $Unread =='1') {
            $UrgentStr = 'Read';
        } elseif ($Urgent == 'Respond' && $Status != 'Unanswered') {
            $UrgentStr = 'Respond to';
        }
        if ($UrgentStr != '') {
            $UrgentStr = '<span style="float:right;" class="red">You must '.$UrgentStr.' this message</span>';
        }
    }
