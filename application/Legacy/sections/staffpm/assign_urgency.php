...
if ($ConvID) {
    ...
    $conv = $master->db->raw_query(
        "SELECT Level, AssignedToUser, UserID, Urgent, Status, Unread FROM staff_pm_conversations WHERE ID=:convid",
        [':convid' => $ConvID]
    ...
    if ($LoggedUser['Class'] >= $conv['Level'] || $conv['AssignedToUser'] == $LoggedUser['ID']) {
        ...
        if ($urgency!='No') {
            if ($conv['Status'] != 'Open') {
                $message .= '[br]'.$sqltime." - (Status automatically set to Open)";
            }
            if ($conv['Unread'] != '1') {
                $message .= '[br]'.$sqltime." - (Message automatically set to Unread)";
            }
        }
