...
function split_tags($list)
{
    ...
    if (!empty($_POST['category'])) {
        $tags = $NewCategories[(int) $_POST['category']]['tag']." ".$tags;
    }

    return explode(' ', strtolower($tags));
}
...
function get_events($EventIDs) {
    ...
    $Events = $master->db->raw_query("Select ID, Title FROM events WHERE ID IN ({$EventIDs}) ORDER BY ID")->fetchAll(\PDO::FETCH_ASSOC);
...
function get_active_events() {
    ...
    if(!$Events=$master->cache->get_value('active_events')) {
        $Events = $master->db->raw_query("Select ID FROM events WHERE StartTime<=? AND EndTime>=? ORDER BY ID", [sqltime(), sqltime()])->fetchAll(\PDO::FETCH_COLUMN);
        $master->cache->cache_value('active_events', $Events, 600);
    }
...
function get_torrent_events($TorrentID) {
    ...
    if(!$Events=$master->cache->get_value("torrent_events_{$TorrentID}")) {
        $Events = $master->db->raw_query("Select e.ID FROM torrents_events AS te JOIN events AS e ON e.ID=te.EventID WHERE TorrentID=?", [$TorrentID])->fetchAll(\PDO::FETCH_COLUMN);
        $master->cache->cache_value("torrent_events_{$TorrentID}", $Events, 600);
    }
...
function get_seed_value($seeders, $size) {
    ...
    return array($seed_value, $seeders_factor, $size_factor);
}
