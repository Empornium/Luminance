...
list($Body, $Image, $GroupID, $GroupName, $GroupCategoryID, $GroupTime) = $TorrentDetails[0];
...
list($TorrentID, $FileCount, $Size, $Seeders, $Leechers, $Snatched, $FreeTorrent, $DoubleTorrent, $TorrentTime,
        ...
        $BadTags, $BadFolders, $BadFiles, $LastReseedRequest, $LogInDB, $HasFile, $IsAnon, $Ducky, $AverageSeeders) = $TorrentList[0];

// update this users last browsed datetime
update_last_browse($LoggedUser['ID'], $TorrentTime);

list($SeedValue, $SeedValueSeeders, $SeedValueSize) = get_seed_value($AverageSeeders, $Size);

$DB->query("SELECT ID FROM articles WHERE TopicID='seedvalue'");
if ($DB->next_record()) {
    $SeedValueText = '<a href="/articles.php?topic=seedvalue">SeedValue</a>';
} else {
...
<div class="details thin">
    <h2>
        <span class="arrow" style="float:left">
            <a href="/torrents.php?id=<?=$GroupID?>&action=prev" title="goto previous torrent"><img src="static/styles/<?=$LoggedUser['StyleName']?>/images/arrow_left.png" alt="prev" title="goto previous torrent" /></a>
        </span>
        <?="$Icons$DisplayName"?>
        <span class="arrow" style="float:right">
            <a href="/torrents.php?id=<?=$GroupID?>&action=next" title="goto next torrent"><img src="static/styles/<?=$LoggedUser['StyleName']?>/images/arrow_right.png" alt="next" title="goto next torrent" /></a>
        </span>
    </h2>
...
<?php           if (!$master->repos->restrictions->is_restricted($LoggedUser['ID'], Luminance\Entities\Restriction::TAGGING) && (check_perms('site_vote_tag') || ($IsUploader && $LoggedUser['ID']==$Tag['userid']))) {  ?>
...
?>
            ...
            <tr class="groupid_<?=$GroupID?> edition_<?=$EditionID?> group_torrent" style="font-weight: normal;" id="torrent<?=$TorrentID?>">
                ...
                <td title="<?= sprintf("Average: %.1f", $AverageSeeders) ?>"><?=number_format($Seeders)?></td>
            ...
            <tr>
                <td colspan="6" class="right" style="border-top:none;border-bottom:none;border-left:none;">
                    <em><?= $SeedValueText ?> <span title="<?= sprintf("SeedFactor %.1f * SizeFactor %.1f = SeedValue %.1f", $SeedValueSeeders, $SeedValueSize, $SeedValue) ?>"><?= sprintf("%.1f", $SeedValue) ?></span></em>
                </td>
            </tr>
            <tr>
                <td colspan="6" class="right" style="border-top:none;border-bottom:none;border-left:none;">
                    ...
                    <tr class="smallhead">
                        <td colspan="2">
                            <span style="float:left;"><a href='#quickreplypreview'>#XXXXXX</a>
                                by <strong><?=format_username($LoggedUser['ID'], $LoggedUser['Username'], $LoggedUser['Donor'], true, $LoggedUser['Enabled'], $LoggedUser['PermissionID'], false, true)?></strong>
                            Just now
                            </span>
            ...
            <div id="details-sidebar" class="content">
                <div id="modal_content">
                  <div class="details thin">
                    <h2>
                        <span class="arrow" style="float: left;">
                            <a href="/torrents.php?id=<?=$GroupID?>&action=prev" title="goto previous torrent">
                                <img src="static/styles/<?=$LoggedUser['StyleName']?>/images/arrow_left.png" alt="prev" title="goto previous torrent" />
                            </a>
                        </span>
                        <?=$GroupName?>&nbsp;(<?=get_size($Size)?>)
                        <span class="arrow" style="float: right;">
                            <a href="/torrents.php?id=<?=$GroupID?>&action=next" title="goto next torrent">
                                <img src="static/styles/<?=$LoggedUser['StyleName']?>/images/arrow_right.png" alt="next" title="goto next torrent" />
                            </a>
                        </span>
                    </h2>
                    <div id="user-sidebar">
...
<?php                               } ?>
                                ...
                                <input type="submit" name="submit" value="Mark for Deletion" disabled="disabled" title="Mark this torrent for Deletion" />
<?php                       } ?>
