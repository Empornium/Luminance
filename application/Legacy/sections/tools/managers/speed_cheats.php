...
if (empty($_GET['order_by']) || !in_array($_GET['order_by'], ['Username', 'upspeed', 'uploaded', 'count', 'time'])) {
...
<?php                       for ($i=2;$i<=20;$i+=2) {
            ...
            <tr class="rowb">
                ...
                <td colspan="2" class="left">
                    ...
                    <input type="checkbox" value="1" onchange="change_view()"
                         id="viewptnupspeed" name="viewptnupspeed" <?php selected('viewptnupspeed', '1', 'checked', $_GET) ?> />
                    &nbsp;&nbsp;
                    <label for="viewptnupload" title="Display records with matching upspeeds">matching uploads</label>
                    <input type="checkbox" value="1" onchange="change_view()"
                         id="viewptnupload" name="viewptnupload" <?php selected('viewptnupload', '1', 'checked', $_GET) ?> />
                    &nbsp;&nbsp;
                    ...
                    <input type="checkbox" value="1" onchange="change_view()"
                         id="viewptnzero" name="viewptnzero" <?php selected('viewptnzero', '1', 'checked', $_GET) ?> />)
                </td>
            </tr>
        </form>
...
$GroupBy = '';
$Having = '';
$viewptnupspeed = $_GET['viewptnupspeed'] ?? 0;
$viewptnupload = $_GET['viewptnupload'] ?? 0;
$viewptnzero = $_GET['viewptnzero'] ?? 0;
if ($viewptnupspeed) {
    $GroupBy .= ", xbt.upspeed";
    $Having = 'HAVING Count(xbt.id)>1';
    if (!$viewptnzero) $WHERE .= " AND xbt.upspeed!=0 ";
}
if ($viewptnupload) {
    $GroupBy .= ", xbt.uploaded";
    $Having = 'HAVING Count(xbt.id)>1';
    if (!$viewptnzero) $WHERE .= " AND xbt.uploaded!=0 ";
}

$Records = $master->db->rawQuery("SELECT SQL_CALC_FOUND_ROWS
                             uid, u.Username, Count(xbt.id) as count, MAX(upspeed) as upspeed, MAX(xbt.uploaded) as uploaded, MAX(mtime) as time,
                             GROUP_CONCAT(DISTINCT LEFT(xbt.peer_id,8) SEPARATOR '|'),
                             GROUP_CONCAT(DISTINCT INET6_NTOA(xbt.ipv4) SEPARATOR '|'),
                             ui.Donor, um.Enabled, um.PermissionID, IF(w.UserID, '1', '0')
                          FROM xbt_peers_history AS xbt
                          JOIN users AS u ON u.ID=xbt.uid
