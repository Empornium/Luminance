...
<?php             } else {
                foreach ($Userlist as $Watched) {
                    list($userID, $Username, $StaffID, $Staffname, $time, $Count, $Comment,
                                       $IsDonor, $enabled, $classID) = $Watched;
                    $row = ($row === 'b' ? 'a' : 'b');
?>

                    <tr class="row<?=$row?>">
                            <td class="center">
                                <a href="?action=speed_records&userid=<?=$userID?>" title="View records for just <?=$Username?>"><img src="/static/common/symbols/view.png" alt="view" /></a>
...
<?php                          }  ?>
                            ...
                            <td class="center"><?=format_username($userID, $IsDonor, true, $enabled, $classID, false, false)?></td>
