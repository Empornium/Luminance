...
$DB->query("SELECT pm.UserID, Username, PermissionID, GroupPermissionID, CustomPermissions, Enabled, Donor, Title
...
$DB->query("SELECT um.ID, Username, PermissionID, GroupPermissionID, CustomPermissions, Enabled, Donor, Title
...
foreach ($UsersInMessages as $UserM) {
    list($PMUserID, $Username, $PermissionID, $GroupPermID, $CustomPermissions, $Enabled, $Donor, $Title) = $UserM;
    $PMUserID = (int) $PMUserID;
    $Users[$PMUserID]['UserStr'] = format_username($PMUserID, $Username, $Donor, true, $Enabled, $PermissionID, $Title, true, $GroupPermID);
...
if (!empty($ReplyID) && $ReplyID!=0 && $CSenderID!=0 && ( !$master->repos->restrictions->is_restricted($LoggedUser['ID'], Luminance\Entities\Restriction::PM) || array_key_exists($ReplyID, $StaffIDs) ) ) {
?>
