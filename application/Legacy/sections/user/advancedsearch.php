...
if (!empty($_GET['search'])) {
    if (preg_match("/^".IP_REGEX."$/", $_GET['search']) && check_perms('users_view_ips')) {
        $_GET['ip'] = $_GET['search'];
        $_GET['ip_history'] = 'on';
...
if (count($_GET)) {
    ...
    if (!$Err) {
        ...
        $OrderTable = array('Username'=>'um1.Username', 'Joined'=>'ui1.JoinDate', 'Last Seen'=>'um1.LastAccess', 'Uploaded'=>'um1.Uploaded', 'Downloaded'=>'um1.Downloaded', 'Ratio'=>'(um1.Uploaded/um1.Downloaded)', 'Invites'=>'um1.Invites', 'Snatches'=>'Snatches');
        ...
        $SQL .= 'um1.PermissionID,
            e1.Address,
            um1.Enabled,
            INET6_NTOA(ip.StartAddress),';
        if (empty($_GET['tracker_ip'])) {
            $SQL .= "'' AS TrackerIP1, ";
        } else {
            $SQL .= "INET6_NTOA(xfu.ipv4) AS TrackerIP1, ";
        }
        $SQL .= 'um1.Invites,
            ui1.Donor,
            ui1.JoinDate,
        ...
        if (!empty($_GET['email']) && check_perms('users_view_email')) {
            ...
            $Where[]='e2.Address'.$Match.wrap($_GET['email']);
        }
        ...
        if (!empty($_GET['ip']) && check_perms('users_view_ips')) {
            if (isset($_GET['ip_history'])) {
                ...
                $Where[]='INET6_NTOA(ip.StartAddress)'.$Match.wrap($_GET['ip'], '', true);
            }
        }
        ...
        if ($_GET['class']!='') {
            if ($Classes[$_GET['class']]['IsUserClass'] == '1') {
                ...
                $Where[]='um1.GroupPermissionID='.wrap($_GET['class'], '=');
            }
        }
...
<div class="thin">
    ...
    <div class="box">
        <div id="as-helpdiv" class="pad" style="display: none;">
            <h4>Joined/Last active</h4>
            <p>Any date field should be in the yyyy-mm-dd format.</p>
...
foreach($results as $result) {
    list($UserID, $Username, $Uploaded, $Downloaded, $Snatched, $Class, $Email, $Enabled, 
         $IP, $trackerIP1, $Invites, $Donor, $JoinDate, $LastAccess) = $result;
?>
        <tr>
            <td><?=format_username($UserID, $Username, $Donor, true, $Enabled, $Class)?></td>
