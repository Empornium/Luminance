...
if (isset($_POST['builddata']) && check_perms('site_debug')) {
    ...
    if ($date_start===false) {
        error("Error in Start date input");
    }
    ...
    if ($deleteend===false) {
        error("Error in End date input");
    }
    if (strtotime($deleteend)<strtotime($date_start)) {
        error("End date is before data range ($deleteend < $date_start)");
    }
