...
if (!empty($_GET['advanced']) && check_perms('site_advanced_top10')) {
    ...
    if (isset($_GET['details'])) {
        if (in_array($_GET['details'], array('day','week','month','year','overall','snatched','data','seeded'))) {
...
if ($Details=='all' || $Details=='month') {
    if (!$TopTorrentsActiveLastMonth = $Cache->get_value('top10tor_month_'.$Limit.$WhereSum)) {
        ...
        $Query .= "
            ...
            LIMIT $Limit;";
        $DB->query($Query);
        $TopTorrentsActiveLastMonth = $DB->to_array(false, MYSQLI_BOTH);
        $Cache->cache_value('top10tor_month_'.$Limit.$WhereSum,$TopTorrentsActiveLastMonth,3600*24);
    }
...
if ($Details=='all' || $Details=='year') {
    if (!$TopTorrentsActiveLastYear = $Cache->get_value('top10tor_year_'.$Limit.$WhereSum)) {
        ...
        $Query .= "
            ...
            LIMIT $Limit;";
        $DB->query($Query);
        $TopTorrentsActiveLastYear = $DB->to_array(false, MYSQLI_BOTH);
        $Cache->cache_value('top10tor_year_'.$Limit.$WhereSum,$TopTorrentsActiveLastYear,3600*24*7);
    }
