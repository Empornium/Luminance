...
list($uploaderID, $title, $groupID, $TorrentID, $totalBounty) = $master->db->raw_query(
    "SELECT r.UserID, r.Title, r.GroupID, r.TorrentID, Sum(rv.Bounty) AS TotalBounty
                                                                          FROM requests AS r
...
if ($returnBounty===true) {
    ...
    foreach ($votes as $vote) {
        $master->db->raw_query(
            "UPDATE users_main SET UPLOADED = (UPLOADED + :bounty) WHERE ID = :voterid",
            [':bounty'  => $vote['Bounty'],
...
if ($TorrentID) {
    $master->cache->delete_value('requests_torrent_'.$TorrentID);
}
if ($groupID) {
