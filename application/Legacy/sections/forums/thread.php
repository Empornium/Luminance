...
if ($_GET['updatelastread'] != '0') {
    ...
    // LastRead can easily get out of sync from forum moderation
    if ($LastRead < $LastPost || $LastRead > $ThreadInfo['LastPostID']) {
...
show_header(display_str($PageTitle), 'comments,subscriptions,bbcode,jquery,overlib');
?>
<div class="thin">
    ...
    <tr class="smallhead">
        <td colspan="2">
            <span style="float:left;"><a class="post_id" href='forums.php?action=viewthread&amp;threadid=<?=$ThreadID?>&amp;postid=<?=$PostID?>#post<?=$PostID?>'>#<?=$PostID?></a>
                <?=format_username($AuthorID, $Username, $Donor, true, $Enabled, $PermissionID, $UserTitle, true, $GroupPermissionID, true)?>
...
if (!$ThreadInfo['IsLocked'] || check_perms('site_moderate_forums')) {
    if (check_forumperm($ForumID, 'Write') && !$master->repos->restrictions->is_restricted($LoggedUser['ID'], Luminance\Entities\Restriction::POST)) {
