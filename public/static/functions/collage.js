...
function CollageSubscribe(collageid) {
  ajax.get("/userhistory.php?action=collage_subscribe&collageid=" + collageid + "&auth=" + authkey, function() {
    ...
    if (subscribeLink) {
      if (subscribeLink.firstChild.nodeValue.trim().substr(1,1) == 'U') {
...
function TrashCollage(collageid, status, token) {
    var toPost = [];
    toPost['status'] = status;
    toPost['token'] = token;
    toPost['reason'] = document.getElementById('remove_reason').value;
    ajax.post("/collage/" + collageid + "/trash", toPost, function (response) {
        jQuery('#collage' + collageid).replaceWith(response);
        jQuery.modal.close();
    });
}
function DeleteCollage(collageid, token = null) {
    var toPost = [];
    toPost['token'] = token;
    toPost['reason'] = document.getElementById('remove_reason').value;
    if (confirm('Are you sure you wish to delete this collage?') == true) {
        ajax.post("/collage/" + collageid + "/delete", toPost, function (response) {
            var x = json.decode(response);
            if (is_array(x)) {
                $('#collage' + collageid).hide();
                jQuery.modal.close();
