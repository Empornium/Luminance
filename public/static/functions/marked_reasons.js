...
function SaveMessage(id) {
	...
	ajax.post("/tools.php?action=mfd_edit_reason", ToPost, function (data) {
                        data = data.trim();
			if (data == '1') {
...
function DeleteMessage(id) {
	...
	var ToPost = [];
	ToPost['id'] = id;
	ajax.post("/tools.php?action=mfd_delete_reason", ToPost, function (data) {
...
function PreviewResponse(id) {
	...
	if ($(div).has_class('hidden')) {
		...
		ajax.post('/tools.php?action=mfd_preview_reason', ToPost, function (data) {
